...
private:
    ...
    VkImageView CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags) {
        ...
        if (vkCreateImageView(logical_device_, &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
    ...
    void CreateDescriptorSets() {
        ...
        for (size_t i = 0; i < kMaxFramesInFlight; i++) {
            ...
            vkUpdateDescriptorSets(logical_device_, static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
        }
    }
    ...
    void CreateTextureImage() {
        ...
        CreateImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, texture_image_, texture_image_memory_);
    ...
    void CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
        ...
        if (vkCreateImage(logical_device_, &imageInfo, nullptr, &image) != VK_SUCCESS) {
        ...
        if (vkAllocateMemory(logical_device_, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
    ...
    void CreateTextureSampler() {
        ...
        if (vkCreateSampler(logical_device_, &samplerInfo, nullptr, &texture_sampler_) != VK_SUCCESS) {
    ...
    void CreateDepthResources() {
        ...
        CreateImage(swap_chain_extent_.width, swap_chain_extent_.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depth_image_, depth_image_memory_);
